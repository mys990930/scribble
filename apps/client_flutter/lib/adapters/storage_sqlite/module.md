...
Drift(SQLite) 기반으로 각 usecase의 Repository 인터페이스를 구현한다.
...
- Repository 구현체 (예: `DriftMemoRepository`)
...
- Repository 인터페이스/서비스 오케스트레이션 정의 → 각 usecase 모듈
- 도메인 비즈니스 규칙 → domain 모듈
- HTTP 통신 → `api-client`
- 웹 플랫폼 데이터 접근 → `api-client`
...
```
storage_sqlite → *_domain (엔티티 타입)
storage_sqlite -.implements.-> *_usecases (Repository)
