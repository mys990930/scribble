...
class MemoWidgetPaddedProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == MemoWidgetProvider.ACTION_REFRESH_MEMO_WIDGET) {
            val manager = AppWidgetManager.getInstance(context)
            val ids = manager.getAppWidgetIds(ComponentName(context, MemoWidgetPaddedProvider::class.java))
    ...
    private fun refreshPendingIntent(context: Context): PendingIntent {
        val refreshIntent = Intent(context, MemoWidgetPaddedProvider::class.java).apply {
    ...
    private fun buildViews(context: Context): RemoteViews {
        val prefs = context.getSharedPreferences(MemoWidgetProvider.PREFS_NAME, Context.MODE_PRIVATE)
        val memosJson = prefs.getString(MemoWidgetProvider.KEY_MEMOS_JSON, "[]") ?: "[]"

        val views = RemoteViews(context.packageName, R.layout.memo_widget_padded)
        views.setTextViewText(R.id.memo_widget_list, memosJsonToStyledLines(memosJson))
        views.setTextViewText(R.id.memo_widget_count, memoCount(memosJson).toString())

        val addIntent = Intent(context, MemoQuickAddActivity::class.java)
        ...
        views.setOnClickPendingIntent(R.id.memo_widget_add_button, addPendingIntent)
        ...
        val openNotesIntent = Intent(context, MainActivity::class.java).apply {
            action = MemoWidgetProvider.ACTION_OPEN_NOTES
            putExtra(MemoWidgetProvider.EXTRA_ROUTE, "/note")
    ...
    private fun memosJsonToStyledLines(json: String): CharSequence {
        return try {
            ...
            contents.take(5).forEachIndexed { i, c ->
    ...
    private fun memoCount(json: String): Int {
        return try {
            Regex("\\\"content\\\"\\s*:\\s*\\\"(.*?)\\\"").findAll(json).count()
        } catch (_: Exception) {
            0
        }
    }
